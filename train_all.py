...
from src.process_all import eval
...
def train(opt):
    ...
    try:
        if torch.cuda.is_available():
            model.load_state_dict(torch.load("{}/ppo_full_{}_{}_{}".format(opt.saved_path, opt.world, opt.stage,opt.saved_episode)))
            model.cuda()
        else:
            model.load_state_dict(torch.load("{}/ppo_full_{}_{}_{}".format(opt.saved_path, opt.world, opt.stage,opt.saved_episode),
    ...
    while True:
        ...
        if os.path.exists('{}/ppo_full_{}_{}_{}'.format(opt.saved_path,opt.world, opt.stage,(curr_episode-1))):
            print('removing past saved data of episode',curr_episode)
            os.remove('{}/ppo_full_{}_{}_{}'.format(opt.saved_path,opt.world, opt.stage,(curr_episode-1)))
        else:
            print('failed to remove past saved model')

        input()
        torch.save(model.state_dict(),
                       "{}/ppo_full_{}_{}_{}".format(opt.saved_path, opt.world, opt.stage, curr_episode))
        curr_episode += 1
        old_log_policies = []
        actions = []
        values = []
